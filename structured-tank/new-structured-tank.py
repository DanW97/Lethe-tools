...
class StructuredStirredTank:
    ...
    def draw(self):
        ...
        for i in range(4):
            ...
            outer_curves.append(
                gm.addCircleArc(outer_split_circle_pts[i], centre, baffle_pts[(i + 1) % 4][3])
            )
            # draw inner curves
            inner_curves.append(
            ...
            inner_curves.append(
                gm.addCircleArc(inner_split_circle_pts[i], centre, baffle_pts[(i + 1) % 4][2])
            )
            ...
            inner_connecting_curves.append(gm.addLine(inner_split_circle_pts[i], outer_square_pts[i]))
            ...
            baffle_extended_edges.append(
                gm.addLine(baffle_pts[i][2], baffle_pts[i][1])
            )
            ...
            outer_square_vertical_edges.append(
                gm.addLine(
                    outside_baffle_pts[i], outer_square_pts[i]
                )
            )
            outer_square_horizontal_edges.append(
                gm.addLine(
                    inside_baffle_pts[i], outer_square_pts[(i - 1)%4]
                )
            )
            middle_straight_edges.append(
                gm.addLine(
                    inside_baffle_pts[i], outside_baffle_pts[i]
                )
            )
        ...
        for i in range(4):
            outer_loop = gm.addCurveLoop([baffle_outside_edges[i], inner_curves[2*i], outer_connecting_curves[i], outer_curves[2*i]], reorient=True)
            outer_curved_surfaces.append(gm.addPlaneSurface([outer_loop]))
            outer_loop = gm.addCurveLoop([outer_connecting_curves[i], inner_curves[2*i+1], baffle_inside_edges[(i+1)%4], outer_curves[2*i+1]], reorient=True)
            outer_curved_surfaces.append(gm.addPlaneSurface([outer_loop]))

            inner_loop = gm.addCurveLoop([outer_outside_straight_edges[i], outer_square_vertical_edges[i], inner_connecting_curves[i], inner_curves[2*i]], reorient=True)
            inner_curved_surfaces.append(gm.addPlaneSurface([inner_loop]))
            inner_loop = gm.addCurveLoop([inner_connecting_curves[i], outer_square_horizontal_edges[(i+1)%4], outer_inside_straight_edges[(i+1)%4],inner_curves[2*i+1]], reorient=True)
            inner_curved_surfaces.append(gm.addPlaneSurface([inner_loop]))

            outer_loop = gm.addCurveLoop([baffle_extended_edges[i], outer_inside_straight_edges[i], middle_straight_edges[i], outer_outside_straight_edges[i]], reorient=True)
            outer_straight_surfaces.append(gm.addPlaneSurface([outer_loop]))

            # print([middle_straight_edges[i], inner_inside_straight_edges[i], centre_square_edges[(i-1)%4], inner_outside_straight_edges[i]])
            inner_loop = gm.addCurveLoop([middle_straight_edges[i], inner_inside_straight_edges[i], centre_square_edges[(i-1)%4], inner_outside_straight_edges[i]], reorient=True)
            inner_straight_surfaces.append(gm.addPlaneSurface([inner_loop]))
            

            square_loop = gm.addCurveLoop([inner_outside_straight_edges[i], inner_inside_straight_edges[(i+1)%4], outer_square_horizontal_edges[(i+1)%4], outer_square_vertical_edges[i]], reorient=True)
            square_surfaces.append(gm.addPlaneSurface([square_loop]))

        inner_square_surfaces.append(gm.addPlaneSurface([gm.addCurveLoop([line for line in centre_square_edges], reorient= True)]))
