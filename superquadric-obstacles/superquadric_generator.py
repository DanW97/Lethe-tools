...
class Superquadric:
    ...
    def __init__(self, indices=[8, 8, 8], scale=[1, 1, 1],
                 filepath='quad', rotation=[0, 0, 0], npts = 20, gr = 1, view = False, init = False) -> None:
    ...
    def export(self, order = 1, refine_passes = 3, optimiser = ''):
        ...
        import os
        os.rename(f"{raw_file}", f"{new_file}")
        if not self.view:
            gmsh.finalize()
        else:
            self._view_mesh()
...
quad = Superquadric(indices=[2, 2, 2], scale= [6.5/1000, 6.5/1000, 3/1000])
quad.draw()
quad.export()