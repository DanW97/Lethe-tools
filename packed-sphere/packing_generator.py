...
class PackingGenerator:
    ...
    to calculate the effect that Reynolds number and packing fraction have on the drag coefficient.
        ...
        """
    def __init__(self, m = 4, Re_max = 100, Vx = 0.005, Vy = 0.005, Vz = 0.005, liggghts_filename = "packingrun", liggghts_folder = "packingsims", sim_filename = "packed_spheres", sim_folder = "lethefiles") -> None:
        self.m = m 
        self.phi_max = 0.3
        self.phi_min = 0.01
        self.Re_max = Re_max 
    ...
    def generate_packing_scripts(self, clean: bool = True):
        ...
        with ThreadPoolExecutor() as exe:
            for i, phi in enumerate(self.setpoints[:,0]):
                filepath = Path(self.liggghts_folder, self.liggghts_filename + f"_{i}.sim")
    ...
    def _write_prm(self, real_phi: float, suffix: int, clean: bool = True):
        ...
        with open(self.sim_header_file, 'r') as f:
            ...
            with open(Path(self.sim_folder, self.sim_filename+f"_{suffix}.prm"), "w") as sim:
                sim.writelines(header)
